...
module.exports = {
  ...
  execute: async (interaction) => {
    ...
    // Split the commands into chunks of 1024 characters
    const discordChunks = splitIntoChunks(discordCommands, 1024);
    const minecraftChunks = splitIntoChunks(minecraftCommands, 1024);

    // Add each chunk as a separate field
    discordChunks.forEach((chunk, index) => {
      infoEmbed.addField(`**Discord Commands ${index + 1}**:`, chunk);
    });

    minecraftChunks.forEach((chunk, index) => {
      infoEmbed.addField(`**Minecraft Commands ${index + 1}**:`, chunk);
    });
